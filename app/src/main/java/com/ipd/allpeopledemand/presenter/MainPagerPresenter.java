...
public class MainPagerPresenter extends MainPagerContract.Presenter {
    ...
    @Override
    public void getMainADImg(TreeMap<String, String> map, boolean isDialog, boolean cancelable) {
        model.getMainADImg(context, map, isDialog, cancelable, getView().bindLifecycle(), new ObserverResponseListener() {
            @Override
            public void onNext(Object o) {
                //这一步是必须的，判断view是否已经被销毁
                if (getView() != null) {
