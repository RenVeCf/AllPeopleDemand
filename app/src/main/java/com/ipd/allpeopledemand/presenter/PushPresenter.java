...
import com.ipd.allpeopledemand.model.PushModel;
...
public class PushPresenter extends PushContract.Presenter {
    ...
    @Override
    public void getPush(TreeMap<String, String> map, boolean isDialog, boolean cancelable) {
        model.getPush(context, map, isDialog, cancelable, getView().bindLifecycle(), new ObserverResponseListener() {
            @Override
            public void onNext(Object o) {
                //这一步是必须的，判断view是否已经被销毁
                if (getView() != null) {
                    getView().resultPush((PushBean) o);
                }
            }
    ...
    @Override
    public void getClassIfication(boolean isDialog, boolean cancelable) {
        model.getClassIfication(context, isDialog, cancelable, new ObserverResponseListener() {
            @Override
            public void onNext(Object o) {
                //这一步是必须的，判断view是否已经被销毁
                if (getView() != null) {
                    getView().resultClassIfication((ClassIficationBean) o);
                }
            }
