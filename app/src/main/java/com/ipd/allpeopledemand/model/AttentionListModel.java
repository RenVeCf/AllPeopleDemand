 ...
 * Time ： 2018/8/26.
 */
public class AttentionListModel<T> extends BaseModel {

    public void getAttentionList(Context context, TreeMap<String, String> map, boolean isDialog, boolean cancelable,
                                 ObservableTransformer<T, T> transformer, ObserverResponseListener observerListener) {

        //当不需要指定是否由dialog时，可以调用这个方法
        //        subscribe(context, Api.getApiService().login(map), observerListener);
        paramSubscribe(context, Api.getApiService().getAttentionList(map), observerListener, transformer, isDialog, cancelable);
    }
